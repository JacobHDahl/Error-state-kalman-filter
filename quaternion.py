...
def quaternion_product(ql: np.ndarray, qr: np.ndarray) -> np.ndarray:
    ...
    #using 10.34
    mid_matrix = np.zeros((4,4))
    skew_sym = utils.cross_product_matrix(epsilon_left)
    mid_matrix[CS.CatSlice(start=1, stop=4) * CS.CatSlice(start=1, stop=4)] = skew_sym
    mid_matrix[CS.CatSlice(start=1, stop=4)*CS.CatSlice(start = 0, stop = 1)] = epsilon_left
    mid_matrix[CS.CatSlice(start=0, stop=1)*CS.CatSlice(start = 1, stop = 4)] = epsilon_left.reshape(1,3)*-1

    left_matrix = eta_left*np.eye(4) + mid_matrix

    quaternion = left_matrix @ q_right #tested and works
...
def quaternion_to_rotation_matrix(
    ...
    #using 10.37: R = I + 2*eta*S(epsilon) + 2*S(epsilon)*S(epsilon)

    skew_epsilon = utils.cross_product_matrix(epsilon)
    R = np.eye(3) + 2*eta*skew_epsilon + 2*skew_epsilon @ skew_epsilon #tested and works (at least the dimensions)
...
def quaternion_to_euler(quaternion: np.ndarray) -> np.ndarray:
    ...
    phi = np.arctan2(2*(epsilon3*epsilon2 + eta*epsilon1),
    quaternion_squared[0]-quaternion_squared[1]-quaternion_squared[2]-quaternion_squared[3])

    theta = np.arcsin(2*(eta*epsilon2-epsilon1*epsilon3))

    psi = np.arctan2(2*(epsilon1*epsilon2 + eta*epsilon3),
    quaternion_squared[0]+quaternion_squared[1]-quaternion_squared[2]-quaternion_squared[3])
