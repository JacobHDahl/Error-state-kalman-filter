...
class ESKF:
    ...
    def predict_nominal(
        ...
        if self.debug:
            assert np.allclose(
                la.norm(quaternion), 1, rtol=0, atol=1e-15
        ...
        a = R @ (acceleration) + self.g #acceleration = am-ab since its debiased
        w =  omega 

        position_prediction = position + Ts*velocity + 0.5*(Ts**2)*a

        velocity_prediction = velocity +Ts*a

        k = Ts*w    #from oppgave
        trig_arg = la.norm(k)/2
        normalized_k = k.T /la.norm(k)
        euler = np.zeros((4,))                     #called it euler because i wanted to
        euler[0] = np.cos(trig_arg)
        euler[1] = np.sin(trig_arg)*normalized_k[0]
        euler[2] = np.sin(trig_arg)*normalized_k[1]
        euler[3] = np.sin(trig_arg)*normalized_k[2] #prøvde serr alt.

        quaternion_prediction = quaternion_product(quaternion, euler)
        ...
        #These correct? Need to discretize these?

        p_omega_b = self.p_gyro
        gyroscope_bias_prediction = gyroscope_bias + Ts * (- p_omega_b * np.eye(3,3) @ gyroscope_bias) #same^
    ...
    def Aerr(
        ...
        skew_sym_acc = utils.cross_product_matrix(acceleration)   #correct? Debiased?
        skew_sym_omega = utils.cross_product_matrix(omega)         #correct? Debiased?
        # Set submatrices
        A[POS_IDX * VEL_IDX] = np.eye(3)
        A[VEL_IDX * ERR_ATT_IDX] = -R @ skew_sym_acc
        A[VEL_IDX * ERR_ACC_BIAS_IDX] = -R
        A[ERR_ATT_IDX * ERR_ATT_IDX] = -skew_sym_omega
    ...
    def Gerr(self, x_nominal: np.ndarray,) -> np.ndarray:
        ...
        G = np.zeros((15, 12))
        lower_matr = la.block_diag(-R,-np.eye(3),np.eye(6))
        upper_matr = np.zeros((3,12))
        G = np.vstack((upper_matr,lower_matr)) #works
    ...
    def discrete_error_matrices(
        ...
        V = np.block([[-A, G@self.Q_err@G.T], [np.zeros_like(A), A.T]])
        assert V.shape == (
            30,
            30,
        ), f"ESKF.discrete_error_matrices: Van Loan matrix shape incorrect {omega.shape}"

        taylor_approx = np.eye(30) + V*Ts + (V*Ts)@(V*Ts)/2  #2nd order taylor, might reduce to first or increase to third
        #taylor_approx = la.expm(V*Ts)  # This can be slow...
        #F = Ad = e^A(Ts) which is in the last column last row of the vanLoan
        #Q = GQGd = V1.T@V2 ??????? #yes.

        V1 = taylor_approx[CatSlice(start=15,stop=30)*CatSlice(start=15, stop=30)]
        V2 = taylor_approx[CatSlice(start=0,stop=15)*CatSlice(start=15, stop=30)]
    ...
    def predict_covariance(
        ...
        P_predicted = Ad @ P @ Ad.T + GQGd      ##GQGd is the same as Q in vanLoans
    ...
    def predict(
        ...
        assert x_nominal.shape == (16,), f"ESKF.predict: x_nominal shape incorrect {x_nominal.shape}"
        ...
        assert z_gyro.shape == (3,), f"ESKF.predict: zGyro shape incorrect {z_gyro.shape}"
        ...
        P_predicted = self.predict_covariance(x_nominal, P, acceleration, omega, Ts) #right?
    ...
    def inject(
        ...
        #The quat-prod is shown in 10.72, so add everything else and quat-prod the quats :)
        x_inj_noquat = x_nominal[INJ_IDX] + delta_x[DTX_IDX]
        err_quat_vec = np.concatenate((np.eye(1),delta_x[ERR_ATT_IDX].reshape(3,1))) #now a column vector

        err_quat_vec = err_quat_vec.reshape(1,4)
        err_quat_vec = err_quat_vec[0]              #now a shape(4,) row-vector

        err_quat_vec = err_quat_vec/la.norm(err_quat_vec) #normalize here as well?

        x_inj_quat = quaternion_product(x_nominal[ATT_IDX],err_quat_vec)
        x_inj_quat = x_inj_quat/la.norm(x_inj_quat) #normalize?
        
        #concatenate to get the whole x_injected
        x_inj_pre = x_inj_noquat[CatSlice(start=0,stop=6)]
        x_inj_post = x_inj_noquat[CatSlice(start=6,stop=len(x_inj_noquat))]

        x_injected = np.concatenate((x_inj_pre,x_inj_quat,x_inj_post)) #works


        #my stuff
        attitude_error = delta_x[ERR_ATT_IDX] * 0.5
        mid_matr_G = np.eye(3) - utils.cross_product_matrix(attitude_error)

        # Covariance
        G_injected = la.block_diag(np.eye(6), mid_matr_G, np.eye(6))
    ...
    def innovation_GNSS_position(
        ...
        H_x = np.zeros((3,16))
        H_x[CatSlice(start=0,stop=3)*CatSlice(start=0,stop=3)] = np.eye(3) #works

        z_est = H_x @ x_nominal

        v = z_GNSS_position - z_est

        # leverarm compensation
        if not np.allclose(lever_arm, 0):
            R = quaternion_to_rotation_matrix(x_nominal[ATT_IDX], debug=self.debug)
            H_x[:, ERR_ATT_IDX] = -R @ cross_product_matrix(lever_arm, debug=self.debug)
            v -= R @ lever_arm

        H = np.zeros((3,15))
        H[CatSlice(start=0,stop=3)*CatSlice(start=0,stop=3)] = np.eye(3)
        S = H @ P @ H.T + R_GNSS
    ...
    def update_GNSS_position(
        ...
        innovation, S = self.innovation_GNSS_position(x_nominal, P, z_GNSS_position, R_GNSS, lever_arm)

        H = np.zeros((3,15))
        H[CatSlice(start=0,stop=3)*CatSlice(start=0,stop=3)] = np.eye(3) #same as above, right?
    ...
    def NIS_GNSS_position(
        ...
        v, S = self.innovation_GNSS_position(x_nominal, P, z_GNSS_position, R_GNSS, lever_arm)
    ...
    def delta_x(cls, x_nominal: np.ndarray, x_true: np.ndarray,) -> np.ndarray:
        ...
        quat_nom = x_nominal[ATT_IDX]
        quat_true = x_true[ATT_IDX]

        real_quat_conj = quat_nom[CatSlice(start=1,stop=4)]*-1
        quat_nom_conj = np.array([quat_nom[0],real_quat_conj[0],real_quat_conj[1],real_quat_conj[2]]) #:)


        delta_quaternion = quaternion_product(quat_nom_conj,quat_true)
        delta_theta = 2*delta_quaternion[CatSlice(start=1,stop=4)]
    ...
    def NEESes(
        ...
        NEES_pos_vel_att = cls._NEES(d_x[POS_IDX+VEL_IDX+ERR_ATT_IDX],P[(POS_IDX+VEL_IDX+ERR_ATT_IDX)**2])

        NEESes = np.array(
            [NEES_all, NEES_pos, NEES_vel, NEES_att, NEES_accbias, NEES_gyrobias,NEES_pos_vel_att]
        )
